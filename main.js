...
module.exports = /*#__PURE__*/JSON.parse('{"projects":[{"id":1,"name":"REST Backend","tasks":[{"id":25,"name":"Push Notifications","done":true},{"id":24,"name":"Test REST API","done":false}]},{"id":2,"name":"Frontend","tasks":[{"id":25,"name":"Push Notifications","done":true},{"id":27,"name":"l118n","done":false}]},{"id":3,"name":"iOS App","tasks":[{"id":25,"name":"Push Notifications","done":true},{"id":26,"name":"Apple Pay Support","done":false},{"id":27,"name":"l118n","done":false},{"id":28,"name":"AppStore Publication","done":false}]}]}');
...
store.state$.subscribe(state => {
  ...
  const curEl = arr.find(element => parseInt(element.dataset.id) === parseInt(state.currentProgectId));
  curEl.style.fontWeight = "900";
});
store.state$.subscribe(state => {
  const currentProjectId = state.currentProgectId;
  const currentProject = state.projects.find(project => project.id === parseInt(currentProjectId));
  ...
  [...statusBtns].forEach(btn => {
    btn.addEventListener("click", e => {
      store.changeStatus(Boolean(e.target.textContent), e.target.dataset.id, currentProjectId);
    });
  });
});
store.state$.subscribe(state => {
  currentProjectChoose.addEventListener("click", () => {
    ...
    const {
      top,
      left
    } = document.body.querySelector(".tasks-component").getBoundingClientRect();
    ...
    document.body.querySelector(".choose-project").addEventListener("click", e => {
